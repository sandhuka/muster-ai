...
- **Sleep-proof**: the driver holds macOS idle-sleep for its own lifetime (`caffeinate -i -w $$`, guarded so non-mac is a no-op) — an overnight run no longer dies when the machine idles.
