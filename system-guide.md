...
- A mechanical `Role: halt` can't self-clear, so resume across a gate uses **`muster/scripts/muster-sprint-resume.sh`**: it has PM process the founder's verdict (insert a fix step per bug, or clear the gate and promote the next wave on approval), then re-enters the loop. Each resume restarts the loop with a fresh step counter, so `MAX_STEPS` naturally scopes per wave-segment.
