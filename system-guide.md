...
**Observability — the run trail.** Each step streams its work (`--output-format stream-json`) through `muster/scripts/muster-sprint-format.sh`. Per-step trail lines are headed by the **queue's own step heading** (`▶ Step 16 — Content: …`) — echoed text from the Next Step block, never parsed for control; the iteration counter is plumbing (the `MAX_STEPS` circuit-breaker) and surfaces only at run start (`run budget: N steps`), near the cap, and in the self-explaining cap-stop message. The live river shows one line per event (📖 read · ✏️ edit · 🧪 bash · ⚠️ error — paths kept, they are the diagnostic trail), and each step closes with an icon-count activity summary plus a telemetry line: `✓ 74 turns · $4.05 · peak ctx 188k/1M 19% · out 21k`. **Peak ctx** is the largest single prompt the step sent — the step-sizing/quality signal (deliberately *not* total input tokens, which are inflated by per-turn cache re-reads and measure cost, not size); **out** is total output tokens. The run ends with a summary table (per-step turns / $ / ctx% / out, plus totals and *why* the run stopped) — scan the ctx% column for oversized-step outliers. Three files per run under `.muster-sprint-logs/`: scannable `run-<ts>.log`, full-fidelity `run-<ts>.jsonl`, machine-readable `run-<ts>.metrics`. All of this is presentation only — the loop's stop conditions key off claude's exit code (`PIPESTATUS[0]`), never the formatter, so a formatter problem can never affect control flow. The driver keeps the most recent `KEEP_RUNS` runs (default 20) and prunes older logs at startup; the log dir is gitignored. Per-step turn budget is `MAX_TURNS` (default 150, env-overridable); a heavy step that exhausts it stops safely (no state advance) — raise `MAX_TURNS` (large but cohesive) or split it along a real seam (`sprint-planning.md` → Size steps by cohesion).
