...
### Muster — the framework agent

The three modes above are how you run *project* work. **Muster** is a separate, opt-in agent that answers for the *framework itself* — it is **not a 9th picker role** and never fires from the picker. One persona, two homes: in a project it is the **Guide**; in the framework repo (where `private/xo/` is present) it is the **XO**. Full loadout in `MUSTER.md` at the muster root; this is the orientation.
...
**Observability — the run trail.** Each step streams its work (`--output-format stream-json`) through `muster/scripts/muster-sprint-format.sh`. Each step opens with a `─── step N` rule and a **role-first header** — `▶ DEVELOPER · Step 16` with the task on the next line (role + heading echoed from the Next Step block, never parsed for control; the iteration counter is plumbing for the `MAX_STEPS` circuit-breaker, surfacing only at run start, near the cap, and in the cap-stop message). The live river shows one line per event (📖 read · ✏️ edit · 🧪 bash · ⚠️ error — paths kept, they are the diagnostic trail), then an icon-count activity summary and the formatter's telemetry line: `✓ 40 turns · $5.39 · peak ctx 199k/1M 19% · out 46k`. **Peak ctx** is the largest single prompt the step sent — the step-sizing/quality signal (deliberately *not* total input tokens, which are inflated by per-turn cache re-reads and measure cost, not size); **out** is total output tokens. The driver then adds a **verified end-block**: `✓ DEVELOPER · 4m 12s · advanced → Step 17 (QA) · handoff HO-042 filed ✓` (the agent's role + this step's wall-clock + the protocol outcome read back from the queue and `agent-requests.md` — independently checked, not the agent's claim), a `run so far: …` cumulative-burn line, a `⚠ ctx ran hot` warning if peak crossed `CTX_WARN_PCT` (default 80), the step-boundary commit line if the floor fired, and a closing rule. The run ends with a summary table (per-step turns / $ / ctx% / out / wall-clock, plus totals and *why* the run stopped) — scan the ctx% column for oversized-step outliers. The agent name shows in its color and key figures bold when output is a terminal (`MUSTER_COLOR`, on by default in a TTY; `0` or `NO_COLOR` opts out so redirected logs stay plain). Three files per run under `.muster-sprint-logs/`: scannable `run-<ts>.log`, full-fidelity `run-<ts>.jsonl`, machine-readable `run-<ts>.metrics`. All of this is presentation only — the loop's stop conditions key off claude's exit code (`PIPESTATUS[0]`), never the formatter, so a formatter problem can never affect control flow. The driver keeps the most recent `KEEP_RUNS` runs (default 20) and prunes older logs at startup; the log dir is gitignored. Per-step turn budget is `MAX_TURNS` (default 150, env-overridable); a heavy step that exhausts it stops safely (no state advance) — raise `MAX_TURNS` (large but cohesive) or split it along a real seam (`sprint-planning.md` → Size steps by cohesion).
...
4. **Non-zero exit** from `claude` → stop for the founder — **except a usage-limit death**, which the driver auto-resumes (see Self-healing run below).
- **`MAX_STEPS`** (default 30, env-overridable) is a cost circuit-breaker.

**Test-gated steps run their test in the foreground.** When a step's acceptance depends on a build or test, the agent must run it **foreground and blocking in a single `Bash` call with an explicit large `timeout`** — never `run_in_background` followed by ending the turn to await the result. The headless `claude -p` step has no inter-turn notification channel: a turn that ends with no pending foreground call exits the process, orphaning the backgrounded test, so the step finishes without filing its handoff or advancing — stop condition 3 then halts the run (visible and lossless, but a human must restart it). The interactive background-completion and `ScheduleWakeup` affordances are no-ops here, and foreground `sleep` is blocked — both quietly tempt the fatal background-then-await pattern; resist it. Run the long test **early** in the step and reserve the tail of the turn budget for *await result → file handoff → advance the queue*, the non-negotiable close of any test-gated step. If a cold build plus the full suite won't fit the `Bash` `timeout` ceiling, add a warm-build step first or scope the run to the affected target — the blocking approach must never trade the notification failure for a timeout failure.

A Next Step block that has a fenced code block but no `Role:` line defaults to `pm` (PM steps may omit the marker); a block with no fenced code block means "complete." An optional **`Model: <model-id>`** line in the block routes that step to a specific model (the driver passes it as `--model`; absent → session default; PM assigns at planning time per `sprint-planning.md` → Prompt standard). The value is only echoed into the flag, never branched on — an invalid id surfaces as a non-zero exit (stop condition 4). The block is bounded at the next `## ` heading **or** an `Upcoming` heading of any level, so the loop reads the true Next Step even in older projects that nest the upcoming list as `### Upcoming`.
...
**Step-boundary commit floor** (driver, end of each successful step): if the step left uncommitted changes, the driver commits them (`sprint step boundary: <step heading>`). Agents committing their own work in closeout remains the convention — one commit per step is the founder's review unit — but the convention is prose, and prose adherence varies (by model, by step). The driver's commit is the deterministic floor: the working tree is committed at every step boundary no matter what, so one step's forgotten commit can never blur into the next step's diff. No-op when the tree is already clean.
...
**Self-healing run (the trio + STATUS).** Three small deterministic pieces let an unattended run survive interruptions that previously needed a human, plus a status file any tab can read:
- **Sleep-proof (idle only)**: the driver holds off macOS *idle* sleep for its own lifetime (`caffeinate -i -w $$`, guarded so non-mac is a no-op) — an overnight run no longer dies to an idle timeout. The `-i` assertion does **not** cover lid-close (clamshell) or low-battery sleep, so long unattended runs want the lid open and AC power; a sleep that does land is non-fatal — the interrupted step is resumable (re-run on wake; mid-step continuation picks up the partial work).
- **Mid-step continuation**: the commit floor guarantees a clean tree at every successful boundary, so a **dirty tree at step start deterministically means an interrupted prior attempt**. The driver prepends a "continue, don't restart" preamble to that step's wrapper prompt (the queue is never edited) and prints `↻`. The fresh agent inventories the partial work and continues. **Deliberate discard stays manual** (`git checkout .` by the founder) — destructive choices are never automated.
- **Limit-aware auto-resume**: a usage-limit death is classified against the run's final result event (`429` + limit text); on a match the driver prints `⏸`, sleeps past the stated reset (+buffer), and re-enters the same step. **Fail-closed** — any classification uncertainty falls through to the normal hard halt (stop condition 4); auto-resume bridges this one mechanical class and **never a `Role: halt` checkpoint**. (Proactive *pre-step* usage-gating was rejected — no reliable usage signal, illusory economics; do not re-derive it.)
- **`$LOGDIR/STATUS`**: one ~8-line file the driver overwrites at run/step start, step end, limit-sleep, and final — run id, state (`running step N` / `sleeping…` / `complete` / `halted: <reason>`), current step label, last completed step's telemetry, totals. It is the **first rung of the Guide's cheap-read ladder** and the only surface that shows *mid-step* state (the trail and metrics show completed work only). Excluded from the commit floor's pathspec, so it never pollutes step commits.
...
**Founder notices (between gates).** Terminal output of an autonomous step is never read by the founder, so an instruction like "surface X to the founder" executed in chat is a swallowed notification. The durable channel is **`knowledge-base/founder-notices.md`**: agents append dated one-line FYIs (parallel-track kickoffs, heads-ups, "X lands at step N"); the driver diffs the file after each step and echoes new entries (📣), counts them in the run summary, and PM folds unread notices into the next `wave-review.md` gate packet — so both the watches-the-terminal and reads-only-at-gates founders see them. The founder deletes notices once acted on. Questions needing **answers** are not notices — they follow the PM escalation path. The driver also alerts (📌) when `## Founder Decisions` changes mid-run (escalated observations park there while the loop keeps running). Queue prompts must use file-action vocabulary, never "tell the founder" (`sprint-planning.md` → Prompt standard).
