...
- **greenfield-discovery.md** — Greenfield (new product) Discovery methodology (5 user-facing stages spread across ~3 sessions): welcome + idea share, market research (Research-owned), go/no-go evaluation, draft review (incl. project root CLAUDE.md populate), Sprint 1 plan. **Mandatory read at bootstrap when `.populated.onboarded_at` is null AND `agents.pm` is null** (greenfield first session).
