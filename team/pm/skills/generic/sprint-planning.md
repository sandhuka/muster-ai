...
9. **Closeout gate (mandatory)**: run `bash muster/scripts/muster-closeout.sh` — the open-items worklist (INFO) plus one ✓/✗ block over board closure, entry bodies, decision reconciliation, kb budgets, durability, and gate-packet notices, in that order; it refuses a green summary if any check failed. Closeout is not complete until it prints ALL GREEN — paste that block into the closeout.
9. Plan next sprint
