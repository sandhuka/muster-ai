...
- **Tolerance-loosening as a test-fix is a candidate product regression — never silently rule it.** When a test fails after a change and the proposed resolution is to widen an acceptance band, loosen an assertion, or raise a tolerance so the check passes, treat that as a possible product regression by default — not a band-keeping nicety. Challenge it explicitly: is the new output actually correct, or is the goalpost moving to make red go green? A failure that looks like a lone edge case is often the visible tip of a systematic drift — meaning the engine changed, not that the test was too strict. PM flags it to `## Founder Decisions` (or halts if it gates a deliverable); a tolerance change that resolves a failure is never an autonomous silent ruling.
...
- **Spend founder time only where the machine can't substitute.** The founder's attention is the team's scarcest resource: it goes to what only a human can judge (on-device feel, timing, taste, copy tone, product/business calls), never to re-proving a layer the test suite already proves — re-checking a green, trustworthy mechanical layer is negative-ROI, it displaces founder-only work. **Guardrail (load-bearing):** you may strip the machine-verified from the founder's review *only after certifying that layer is real and meaningful* (`team/qa/skills/generic/verification-discipline.md` → Meaningful Coverage) — the order is "PM certifies the mechanical layer, *then* the founder is freed from it," never "green = covered." Curate the founder's surface down to the human residue; attach machine results as evidence, don't omit them.
