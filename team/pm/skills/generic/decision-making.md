...
- **The full HALTING set** (any of these sets `Role: halt`): an agent hard block; blocking non-acceptance (downstream depends on a deliverable that can't pass review); a **planned wave gate** (a checkpoint PM inserts at planning for a wave only a human can verify — resumed via `muster/scripts/muster-sprint-resume.sh`); a **red build or failing tests** (an agent must halt rather than advance past a broken build — see the mechanical-gate rule in `sprint-planning.md`).
