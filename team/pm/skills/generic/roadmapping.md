...
- The goal is to prove the core value proposition: smart daily routine generation works and users trust it
...
- Prioritize features that:
  - Improve retention (offline download, enhanced personalization)
  - Expand addressable market (Android)
  - Add high-demand content (HIIT discipline)
  - Enable user control (manual routine building)

### v2.0+ (Platform Expansion)
- Nice to Have features, loosely ordered by strategic value
- Larger scope items: Apple Watch, new disciplines (Pilates, HIIT), tvOS
- Social features and advanced analytics
- Sequence by: strategic value, technical readiness, market demand signals

## Sequencing Principles

1. **Ship the smallest thing that proves the core value prop.** The MVP proves that smart daily routine generation is compelling enough to retain users and convert them to paid. Every feature in MVP should serve this proof.

2. **Content pipeline and algorithm are parallel workstreams.** Exercise content creation (3D animation) and scheduling algorithm development can progress simultaneously. Plan them as such — they don't block each other until integration testing.

3. **New disciplines are content additions, not architectural changes.** The architecture should support adding HIIT, Pilates, or other disciplines without code changes beyond the content itself and discipline-specific scheduling rules. Plan content expansion accordingly.
...
1. **Core loop test**: Without this, the product's core value proposition (smart daily routine generation) cannot be demonstrated. If a user can experience the product's differentiator without it, it's not Must Have.
2. **Retention test**: Without this, a user who completes their first routine has no reason to return tomorrow. If it doesn't drive Day 2 retention, it's probably a Should Have.
