...
## Purpose
Define the diagnostic workflows behind the Core Web Vitals budgets that `web-best-practices.md` only states. Bundle analysis with CI regression detection, client-component minimization with concrete refactorings, `dynamic()` import patterns, request-waterfall elimination, React DevTools Profiler workflow, `next/font` and `next/image` discipline on the LCP element, third-party script isolation. The targets live in `web-best-practices.md`; this skill is how to hit them. See the `web-best-practices` skill for the budgets. See the `web-modern-react` skill for effects discipline, Suspense, React Compiler. See the `web-state-management` skill for re-render prevention via selectors. See the `web-nextjs-app-router` skill for built-in optimizations. See the `web-cicd` skill for Lighthouse CI wiring. Target: **Next.js 15+, React 19+**.
