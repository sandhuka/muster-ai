...
NEW_ENTRIES='[
  ...
  "Bash(bash muster/scripts/muster-plan-gate.sh)",
  "Bash(bash muster/scripts/muster-closeout.sh)",
  "Bash(bash muster/scripts/muster-lint-step.sh:*)",
  "Bash(bash muster/scripts/muster-lint-sprint.sh:*)"
]'
...
if [ "$PROJECT_EXISTS" -eq 1 ]; then
    ...
    if [ "$MISSING" = "0" ]; then
        skip ".claude/settings.json permissions" "all 18 entries already present"
    else
        if [ "$DRY_RUN" -eq 1 ]; then
    ...
    if [ "$USER_MISSING" = "0" ]; then
        skip "project-level skipped" "user-level $USER_SETTINGS already has all 18 entries"
    else
        STEP2_DEFERRED=1
        say ""
        ...
        say '  "Bash(bash muster/scripts/muster-plan-gate.sh)",'
        say '  "Bash(bash muster/scripts/muster-closeout.sh)",'
        say '  "Bash(bash muster/scripts/muster-lint-step.sh:*)",'
        say '  "Bash(bash muster/scripts/muster-lint-sprint.sh:*)"'
        say ""
...
if [ "$DRY_RUN" -eq 1 ]; then
    ...
    say "Permission prompts will continue to fire until you add the 18 entries to your user-level settings."
    say ""
