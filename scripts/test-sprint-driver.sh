...
ok "per-step wall-clock in end-block"     'grep -aq "✓ QA · 0s ·" "$TEST/runA.out" && grep -aq "run so far:" "$TEST/runA.out"'
ok "step-progress: role + advance + HO"   'grep -aq "✓ QA · 0s · advanced → Step 2 — PM: fixture review (PM) · handoff HO-001 filed ✓" "$TEST/runA.out"'
