...
printf "  %sWhat happens next — 6 stages, ~2 hours total:%s\n\n" "$BOLD" "$RESET"
printf "    %sStage 1%s  Brain-dump      (~25 min)  %s← highest leverage%s\n" "$BOLD" "$RESET" "$YELLOW" "$RESET"
printf "    %sStage 2%s  Code audit      (~15 min)  %s(mostly waiting)%s\n" "$BOLD" "$RESET" "$DIM" "$RESET"
printf "    %sStage 3%s  Audit review    (~20 min)  %s← your focus%s\n" "$BOLD" "$RESET" "$YELLOW" "$RESET"
printf "    %sStage 4%s  Questionnaire   (~15 min)\n" "$BOLD" "$RESET"
printf "    %sStage 5%s  Draft review    (~15 min)\n" "$BOLD" "$RESET"
printf "    %sStage 6%s  Sprint 1 plan   (~5 min)\n\n" "$BOLD" "$RESET"

printf "  %sClaude opens with a short welcome before Stage 1.%s\n\n" "$DIM" "$RESET"
