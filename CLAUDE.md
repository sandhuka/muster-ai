...
14. **Bootstrap context budget** — PM bootstrap reads (all 7 monitoring files + PM brain file) should stay under 600 lines. To enforce this: (a) `current-sprint.md` contains only the active sprint — completed sprints are moved to `sprint-archive.md` at sprint closeout, (b) `decision-log.md` keeps only entries from the current sprint — older entries are moved to `decision-log-archive.md` at sprint closeout. If bootstrap reads exceed 600 lines despite archiving, investigate which file is growing and trim or archive further.
